...
serve(async (req) => {
  ...
  try {
    console.log('🚀 TRELLO SYNC FUNCTION - VERSION 3.0 WITH SMART DEDUPLICATION');
    ...
    console.log('🔍 DEBUG - Auth string:', authString);
    console.log('🔍 DEBUG - List ID:', list_id);
    console.log('🔍 DEBUG - Full URL:', trelloUrl);
    console.log('🔍 DEBUG - API Key length:', trelloApiKey?.length || 0);
    console.log('🔍 DEBUG - Token length:', trelloToken?.length || 0);
    console.log('🔍 DEBUG - Trello API Key value:', trelloApiKey);
    console.log('🔍 DEBUG - Trello Token value:', trelloToken);
    ...
    try {
      ...
      if (!cardsResponse.ok) {
        ...
        console.error('🔍 DEBUG - Full error response:', {
          ...
          headers: Object.fromEntries(cardsResponse.headers.entries()),
          body: errorText
        })
    ...
    console.log(`🔍 First card example:`, {
      title: links[0]?.title,
      trello_card_id: links[0]?.metadata?.trello_card_id,
      url: links[0]?.url
    })
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/trello-sync-v2' \
